...
export default function SpecializationDetailsPage() {
  ...
  return (
    <div className="text-white font-sans selection:bg-[#00ff94] selection:text-black">
      
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="relative h-[400px] md:h-[550px] overflow-hidden rounded-[2rem] md:rounded-[40px] border border-white/5 shadow-2xl">
          <img
          ...
          <div className="absolute inset-0 bg-gradient-to-r from-black/90 via-black/60 lg:via-black/40 to-transparent flex flex-col justify-center px-6 md:px-12 lg:px-20">
            <div className="flex items-center gap-2 mb-3 lg:mb-4">
               <div className="w-2 h-2 rounded-full bg-[#00ff94] animate-pulse" />
               <span className="text-[#00ff94] text-[9px] lg:text-[10px] font-black tracking-widest uppercase italic">Recommended Session</span>
            </div>

            <h1 className="text-3xl md:text-5xl lg:text-7xl font-black mb-4 lg:mb-6 max-w-2xl leading-[1.1] uppercase italic tracking-tighter">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 lg:py-16 grid lg:grid-cols-3 gap-8">
        
        <div className="lg:col-span-2">
          <div className="bg-[#0d1f17] p-6 md:p-12 rounded-[2rem] md:rounded-[2.5rem] border border-white/5 relative overflow-hidden h-full">
             ...
             <div className="flex items-center gap-3 mb-6 md:mb-8 relative z-10">
                ...
                <h2 className="text-xl md:text-2xl font-black uppercase italic tracking-tight leading-none">About Program</h2>
             </div>
             <p className="text-gray-400 text-sm md:text-lg leading-relaxed italic relative z-10 font-medium break-words">
                {specialization?.description}
             </p>
          </div>
        </div>

        <div className="bg-[#0d1f17] p-6 md:p-10 rounded-[2rem] md:rounded-[2.5rem] border border-white/5 overflow-hidden">
          <div className="flex items-center gap-3 mb-8 md:mb-10">
             <div className="w-3 h-3 md:w-4 md:h-4 bg-[#00ff94] rounded-sm" />
             <h2 className="text-xl md:text-2xl font-black uppercase italic tracking-tight leading-none">Structure</h2>
          </div>

          <div className="space-y-0 relative">
            {workoutList.map((wk: { id: string; title: string; duration: number; description: string; }, index: number) => (
              <div key={wk.id} className="relative flex gap-4 md:gap-6 pb-12 last:pb-0">
                {index !== workoutList.length - 1 && (
                  <div className="absolute left-[19px] top-10 bottom-0 w-[1px] bg-white/10" />
                )}
                <div className="relative z-10 w-10 h-10 rounded-full bg-black flex items-center justify-center border border-white/10 text-[#00ff94] font-black text-[10px] shrink-0">
                  {String(index + 1).padStart(2, '0')}
                </div>
                <div className="flex-1 pt-1 min-w-0">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 mb-2">
                    <h3 className="font-black uppercase italic text-sm text-white tracking-tighter break-words leading-tight">{wk.title}</h3>
                    <span className="text-[#00ff94] text-[9px] font-black uppercase tracking-widest shrink-0">{wk.duration} Min</span>
                  </div>
                  <p className="text-gray-500 text-xs italic font-medium leading-relaxed break-words">{wk.description}</p>
                </div>
              </div>
            ))}
          </div>
