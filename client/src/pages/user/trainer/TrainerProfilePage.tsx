...
export default function TrainerProfilePage() {
  ...
  return (
    <div className="space-y-6 md:space-y-10 p-4 md:p-0">
      ...
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 md:gap-8">
        <div className="flex items-center gap-4 md:gap-6 w-full lg:w-auto">
          <div className="relative group shrink-0">
            <div className="w-16 h-16 md:w-24 md:h-24 rounded-full border-2 border-[#00ff94] p-1 shadow-[0_0_20px_rgba(0,255,148,0.2)] overflow-hidden bg-black">
              <img
                src={avatarSrc}
            ...
            <label className="absolute bottom-0 right-0 bg-[#00ff94] p-2 rounded-full text-[#0d1f17] hover:scale-110 transition-transform cursor-pointer shadow-lg">
              <Edit3 size={10} className="md:w-[14px] md:h-[14px]" />
              <input
                type="file"
                className="hidden"
                accept="image/*"
          ...
          <div className="min-w-0">
            <h1 className="text-xl md:text-4xl font-black italic uppercase tracking-tighter text-white leading-none">Account settings</h1>
            <div className="flex flex-wrap items-center gap-2 mt-2">
              <span className="px-2 md:px-3 py-1 bg-[#00ff94]/10 text-[#00ff94] text-[8px] md:text-[9px] font-black uppercase italic rounded-lg flex items-center gap-1 shrink-0">
                <ShieldCheck size={10} /> {trainer.status}
              </span>
              {trainer.specializationName && (
                <span className="px-2 md:px-3 py-1 bg-white/5 text-gray-400 text-[8px] md:text-[9px] font-black uppercase italic rounded-lg border border-white/5 truncate">
                  {trainer.specializationName}
                </span>
              )}
            </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
          {!isEditing && (
            <Link
              to="/trainer/wallet"
              className="w-full sm:w-auto flex items-center justify-center gap-2 bg-white/5 border border-white/10 px-6 py-3 md:py-4 rounded-2xl font-black uppercase text-white italic text-[10px] md:text-xs hover:bg-white/10 transition-all"
            >
      ...
      {/* Profile Card */}
      <div className="bg-[#0a1810] border border-white/5 rounded-3xl md:rounded-[3rem] p-6 md:p-14 shadow-2xl">
        <form className="grid grid-cols-1 md:grid-cols-2 gap-x-8 md:gap-x-12 gap-y-6 md:gap-y-10">
          <TrainerProfileField
            icon={<User size={16} />}
            label="First Name"
            name="firstName"
        ...
        <div className="h-px bg-white/5 w-full my-8 md:my-12" />

        {/* Password Security Box */}
        <div className="flex flex-col sm:flex-row items-center justify-between p-5 md:p-6 bg-black/20 rounded-2xl md:rounded-3xl border border-white/5 group hover:border-[#00ff94]/30 transition-all gap-4 sm:gap-0">
